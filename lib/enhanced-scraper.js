 ...
 */
export async function extractName(page, url) {
  try {
    ...
    if (rootDomain !== url) {
      try {
        ...
        await rootPage.goto(rootDomain, { waitUntil: 'domcontentloaded', timeout: 15000 });
 ...
 */
export async function scrapeOrganisation(url, page) {
  try {
    const data = {
      name: await extractName(page, url),
      website: url,
