...
// Provider configuration with endpoints, models, and environment variable names
const PROVIDERS = {
  openrouter: {
...
/**
 * Main classification function with multi-provider fallback
 * 
 * Attempts to classify an organisation using multiple AI providers in order:
 * 1. OpenRouter
 * 2. HuggingFace
 * 3. Moonshot
 * 
 * Falls back to the next provider if the current one fails or returns invalid results.
 ...
 *   provider: string ("openrouter" | "huggingface" | "moonshot") or null,
...
async function classifyWithAI(orgData, options = { enableGracefulDegradation: true }) {
  ...
  // Try providers in order: OpenRouter, Cerebras, Moonshot
  const providers = [
  ...
  for (const provider of providers) {
    try {
      console.log(`Attempting classification with ${provider.name}...`);
      const result = await provider.fn(orgData);
      
      if (result) {
        console.log(`Successfully classified with ${provider.name} (model: ${result.model}, confidence: ${result.confidence})`);
      ...
      console.log(`Provider ${provider.name} failed or returned invalid result, trying next provider...`);
    } catch (err) {
      console.error(`Provider ${provider.name} threw error: ${err.message}`);
  ...
  if (options.enableGracefulDegradation) {
    console.log(`Using default classification for ${orgData.name} (will be flagged for review)`);
